...
### 8.12 Motion & lighting layer

An ambient/interactive layer over the storefront's static visual language (§2.2), added as pure presentation with no effect on data flow. Split deliberately between CSS-only effects (no client JS at all) and JS-driven ones (each independently checking `window.matchMedia("(prefers-reduced-motion: reduce)")`, since CSS keyframe/transition-based effects are already neutralized site-wide by the `prefers-reduced-motion` block in `globals.css`, which forces `animation-duration`/`transition-duration` to ~0):

- **CSS-only** (Server Components, `:hover`/`:focus-visible`-driven): `ScribbleLink.tsx` (draw-on underline via `stroke-dasharray`/`stroke-dashoffset`, used in `Header.tsx`/`Footer.tsx`); `.glitch-text` in `globals.css` (RGB-split via `::before`/`::after` + `clip-path` keyframes, applied via `className="glitch-text"` + `data-text="..."` on the homepage hero badge and `TagPill.tsx`'s sale pill). `.glitch-text` deliberately doesn't declare its own `position` — the ghost layers just need *some* positioned ancestor, and forcing `relative` in the shared rule would silently override a caller that's already `position: absolute` (same specificity, and the rule sits after Tailwind's `@import` in cascade order, so it would win) — each call site supplies its own positioning instead.
- **JS-driven** (`"use client"`): `Button.tsx` (magnetic pointer-pull + glow, `pointermove`/`pointerleave`, skipped for touch and reduced motion — the pull is folded into the same inline `transform` the brand's `rotate(-1deg)` primary-button tilt uses, rather than a wrapper element, so it doesn't break `w-full`/`flex-1` layouts built on `Button`'s `className`); `ProductCard.tsx` (`IntersectionObserver`-based scroll reveal, defaulting to visible so it degrades safely with JS off or a slow observer callback, plus pointer-driven 3D tilt via `perspective()/rotateX()/rotateY()` on the image frame); `ScrambleHeadline.tsx` (character-cycling decode-in on the homepage hero headline, `setInterval`-driven, renders the real text immediately so SSR/no-JS output is unaffected — the scramble is a flourish layered on top, never the only copy of the words); `AddToCartForm.tsx` (a short-lived colored ink-splat at the click point on Add to cart/Preorder, and a `key`-remounted `preorder-glow` pulse on the preorder banner that only replays when the *selected variant* changes, not on unrelated re-renders).
- **Ambient, storefront-wide**: `.site-wall` (a fixed scatter of the same circle/square/X marks `PlaceholderFrame` draws, `z-index: -1` so it's always painted behind normal content regardless of what else on the page creates a stacking context), `.site-grain` (a fixed, animated film-grain overlay), and `MarqueeTicker.tsx` (an endless CSS `translateX` brand strip, duplicated track for a seamless loop) — all mounted once in `(storefront)/layout.tsx` alongside `ChatBubble.tsx`/`CartDrawer` so no page has to think about them.
- **Crayon scribble marks** — `CrayonScribble.tsx`, a Server Component: a small `<g>` of parallel strokes run through one SVG filter (`feTurbulence` → `feDisplacementMap`) for a hand-scribbled, slightly wobbly crayon texture instead of a flat stroke. Each usage passes a distinct `id` (only needs to be unique among instances on the same page — it seeds the filter and the noise pattern) plus a brand-token `color`. Scattered across open space at six spots: the homepage hero (bottom-right of the left column), all three homepage section headers (Collections/Featured drop/New In, next to each `Crown`), the gender-hub PLP header (`GenderHub.tsx`), and the footer's top row (`Footer.tsx`).
- **Button hover scribble** — `Button.tsx` renders `Scribble.tsx`'s existing `"underline"` shape (already used for the arrow icon in another shape) as an absolutely positioned child (`.btn-scribble-underline` in `globals.css`, `left:0; right:0` so it spans the button's full width edge to edge), invisible until hover/focus via the same `stroke-dasharray`/`stroke-dashoffset` draw-on `ScribbleLink.tsx` already uses. The underline SVG carries `preserveAspectRatio="none"` so it stretches to match each button's actual width/height ratio instead of centering at its own fixed aspect ratio — without that, a wide full-width button (e.g. "Add to cart") would show a short centered scribble rather than one spanning the button. Which brand color a given button gets is decided once, deterministically, by hashing `${variant}:${href ?? label}` (a small string hash, `pickScribbleColor()`) — stable across renders and SSR/client (no hydration mismatch, unlike `Math.random()`), but different across buttons since the seed differs. The color pool excludes that variant's own dominant color (lime, for both `primary`'s fill and `ghost`'s hover border) so the scribble always reads as a distinct accent rather than blending into the button's own hover state. Scoped to `variant !== "text"` (the same condition already used for the magnetic-pull effect) — plain inline text links keep their simple `hover:underline` instead. This covers every call site of the shared `Button` component (all the real storefront CTAs — Shop now, Add to cart, Preorder, Notify me, Checkout, etc.); smaller raw `<button>` elements in account/admin forms (saves, qty steppers, accordion toggles) don't get it, by design — the effect is scaled for a primary call-to-action. (An earlier version of this also had a rough oval scribbled around the button as an alternate treatment on about a third of buttons; removed per feedback in favor of a single consistent underline treatment, sized correctly.)

All of the motion/lighting CSS lives in one block at the end of `globals.css` ("Motion & lighting"), reusing the existing brand tokens (`--color-ic-lime`/`-pink`/`-cyan`/`-yellow`) rather than introducing new colors.
