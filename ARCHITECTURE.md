...
- **Button hover scribble** — `Button.tsx` extends `Scribble.tsx` (already used for the arrow icon) with a new `"oval"` shape, then renders either that or the existing `"underline"` shape as an absolutely positioned child (`.btn-scribble-underline` / `.btn-scribble-oval` in `globals.css`), invisible until hover/focus via the same `stroke-dasharray`/`stroke-dashoffset` draw-on `ScribbleLink.tsx` already uses. Which shape and which brand color a given button gets is decided once, deterministically, by hashing `${variant}:${href ?? label}` (a small string hash, `pickScribble()`) — stable across renders and SSR/client (no hydration mismatch, unlike `Math.random()`), but different across buttons since the seed differs. Roughly one in three buttons gets the oval treatment; the rest get the underline. Scoped to `variant !== "text"` (the same condition already used for the magnetic-pull effect) — plain inline text links keep their simple `hover:underline` instead. This covers every call site of the shared `Button` component (all the real storefront CTAs — Shop now, Add to cart, Preorder, Notify me, Checkout, etc.); smaller raw `<button>` elements in account/admin forms (saves, qty steppers, accordion toggles) don't get it, by design — the effect is scaled for a primary call-to-action.
