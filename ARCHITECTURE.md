...
- **Ambient, storefront-wide**: `.site-grain` (a fixed, animated film-grain overlay, one div mounted once in `(storefront)/layout.tsx`) and `MarqueeTicker.tsx` (an endless CSS `translateX` brand strip, duplicated track for a seamless loop), both mounted alongside `ChatBubble.tsx`/`CartDrawer` in the storefront layout so no page has to think about them.
